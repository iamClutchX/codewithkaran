...
const ProjectDetailTemplate = ({ project }) => {
  ...
  return (
    <div className="flex p-16 flex-col md:flex-row bg-gray-100 dark:bg-gray-800 rounded-lg shadow-md overflow-hidden mb-8 w-full text-justify">
      ...
      <div className="w-full md:w-1/2 p-6 space-y-2">
        ...
        <div className="flex flex-wrap">
          {(showAllTech ? project.technologies : limitedTech).map(
            (tech, index) => (
              <span
                key={index}
          ...
          {project.technologies.length > 3 && (
           ...
           hover:text-primary  dark:hover:hover:text-primary 
            "
            >
              {showAllTech ? "Less" : `More ${project.technologies.length - 3}`}{" "}
              <AiOutlineArrowDown className="inline-block ml-1" />
            </button>
          )}
        </div>
